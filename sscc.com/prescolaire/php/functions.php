<?php
	class login_registration_class{
		...
		//ALL FUNCTION FOR FACULTY

		// funcction for faculty registration
		public function fct_registration($name,$uname, $pass,$email, $bday,$gender,$edu,$contact,$address){
			global $conn;
			$fct = $conn->query("select id from class where username='$uname' ");
			$count = $fct->num_rows;
			if($count == 0){
				$sql = "insert into class(name,username,password,email,birthday,gender,education,contact,address) values('$name','$uname','$pass','$email','$bday','$gender','$edu','$contact','$address')";
				$result = $conn->query($sql);
				return true;
			}else{
				return false;
		...
		//function to get faculty by usernamme 
		public function get_faculty_by_username($uname){
			global $conn;
			$sql = "select * from class where username='$uname'";
			$result = $conn->query($sql);
			return $result;
		}
		...
		//fucntion for login for faculty 
		public function fct_login($uname, $pass){
			global $conn;
			$sql = "select id,username,name from class where username='$uname' and password='$pass' ";
			$result = $conn->query($sql);
			$count = $result->num_rows;
		...
		//function for get facukty session
		public function get_faculty_session(){
			return @$_SESSION['fct_login'];
		}
		//END FUNCTION FACULTY

		//ALL FUNCTIO FOR ADMIN 

		//function for getting All student infomation 
		...
		public function search($query){
			global $conn;
			$result = $conn->query("SELECT * FROM st_info WHERE (st_id LIKE '%".$query."%'OR name LIKE '%".$query."%'OR contact LIKE '%".$query."%'OR email LIKE '%".$query."%') order by st_id");
			return $result;
		}
		...
		//function for getting admin session
		...
		//function for deleting attendance
		...
		//function for get attendance date 
		...
		//function fior updating student result
